...
echo "Converting PDB to CIF format" >&2
...
{
  "cli_tool": "maxit",
  "arguments": ["-input", "input.pdb", "-output", "output.cif", "-o", "1"],
  "files": [
    {
...
echo "$RESPONSE" | jq -r '.files[] | select(.relative_path == "output.cif") | .content'
