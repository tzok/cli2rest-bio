...
def load_tool_config(tool_name):
    ...
    if not os.path.exists(config_path):
        print(f"Error: Configuration for tool '{tool_name}' not found at {config_path}", file=sys.stderr)
        sys.exit(1)
...
def parse_arguments(config):
    ...
    parser = argparse.ArgumentParser(description=config.get('description', 'Tool runner'))
    ...
    for input_param in config.get('inputs', []):
        ...
        if input_param.get('type') == 'boolean':
            parser.add_argument(
                ...
                action='store_true' if input_param.get('default', False) else 'store_false',
...
def find_input_files(file_path, extensions):
    ...
    if os.path.isfile(file_path):
        ...
        if any(file_path.lower().endswith(ext.lower()) for ext in extensions):
            ...
            print(f"Error: Input file must have one of these extensions: {valid_exts}", file=sys.stderr)
            sys.exit(1)
        ...
        if not files:
            valid_exts = ", ".join(extensions)
            print(f"Error: No files with extensions {valid_exts} found in directory '{file_path}'", file=sys.stderr)
            sys.exit(1)
...
def process_file(input_file, config, args, port):
    ...
    for output in config.get('outputs', []):
        ...
        output_path = os.path.join(input_dir, render_template(output_pattern, variables))
        ...
        if os.path.exists(output_path):
            print(f"Skipping {input_file} - output file already exists: {output_path}", file=sys.stderr)
            return
...
def main():
    ...
    tool_name = script_name.split('-')[0] if '-' in script_name else script_name.split('.')[0]
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=args.threads) as executor:
            futures = [executor.submit(process_file, input_file, config, args, port) 
