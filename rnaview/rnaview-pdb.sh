...
CONTAINER_NAME="rnaview-$(date +%s)"

echo "Starting RNAView container..." >&2
# Start the container in the background with a random port
docker run -d --name "$CONTAINER_NAME" -P ghcr.io/tzok/cli2rest-rnaview:latest >&2
...
EOF
	...
	# Extract the output files and save them
	echo "$RESPONSE" | jq -r '.files[] | select(.relative_path != "input.pdb") | "\(.relative_path)\n\(.content)"' | while read -r FILE_PATH && read -r CONTENT; do
		OUTPUT_FILE="$OUTPUT_DIR/${FILENAME}_${FILE_PATH}"
		echo "$CONTENT" > "$OUTPUT_FILE"
		echo "Saved output to: $OUTPUT_FILE" >&2
	done
	
	# Also save stdout and stderr for debugging
	echo "$RESPONSE" | jq -r '.stdout' > "$OUTPUT_DIR/${FILENAME}_stdout.txt"
	echo "$RESPONSE" | jq -r '.stderr' > "$OUTPUT_DIR/${FILENAME}_stderr.txt"
}
